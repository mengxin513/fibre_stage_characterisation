    ...
    with picamera.PiCamera(resolution=(3280/2,2464/2)) as camera, \
        ...
        for startpos, fname in zip(starting_positions, foldernames):
            ...
            if np.any(startpos != stage.position):
                stage.move_rel(startpos - np.array([-1,-1,0])*backlash - stage.position)
                stage.move_rel(np.array([1,1,0])*backlash)

#            ms.autofocus(dz_array(100,10), backlash=backlash)
                    
            for i in ms.scan_z(log_dz_array(50,1.2, 15), backlash=backlash):
                time.sleep(1)
                camera.capture(os.path.join(output_dir,fname,"edge_zstack_%03d_x%d_y%d_z%d.jpg" % ((i,) + tuple(stage.position))), use_video_port=True)
        time.sleep(0.5)
